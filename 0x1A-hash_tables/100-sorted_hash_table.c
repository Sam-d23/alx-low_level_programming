 ...
 *
 * Return:  NULL (failure).
 *        a pointer to the new sorted hash table (Success).
 */
shash_table_t *shash_table_create(unsigned long int size)
{
	shash_table_t *ht;
...
/**
 * shash_table_set - function that adds an
 ...
 * @ht: pointer to the sorted hash table.
 * @key: The key whose value we seek.
 *
 * Return: NULL (failure).
 *         the value associated with key in ht (Success).
 */
char *shash_table_get(const shash_table_t *ht, const char *key)
{
	shash_node_t *node;
